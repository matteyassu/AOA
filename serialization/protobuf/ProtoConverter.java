...
/*
convert object to protobuffer format and write to txt file. Then stick txt file into bytebuffer somehow, then transfer over to hikey\
On hikey end, re-inflate txt to object and manipulate

Protobuf may not be best solutions for few reasons
   no existing library to automatically convert obj -> protobuf.txt 
*/
      ...
      for(Method m : methods){
         if(m.getName().substring(0,3).equals("get") && m.getParameterTypes().length == 0){
            if(m.getName().equals("getClass"))
               continue;
            Object r = m.invoke(o);
            String dataType = (r.getClass() + "").substring((r.getClass() + "").lastIndexOf(".") + 1);
            String protoKw = namingConventions.get(dataType);
            String dataName = " " + m.getName().substring(3,4).toLowerCase() + m.getName().substring(4) + " = " + i++ + ";\n";
            boolean builtIn = (protoKw == null) ? false:true;
            if(builtIn)//built-in
               s += protoKw + dataName;
            else{
               if(namingConventions.get(methods.get(methods.size()-1).invoke(o).) != null){
                  methods.remove(m);
                  methods.add(m);//add to end
               }
               else{
